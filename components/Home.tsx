...
const Home: React.FC<Props> = ({ onStart }) => {
  return (
    <div className="flex flex-col items-center justify-center h-full p-6 relative overflow-hidden">
      ...
      <div className="text-center space-y-8 z-10 max-w-2xl animate-fade-in">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-surfaceContainer dark:bg-surfaceContainer-dark border border-outline/10 text-primary dark:text-primary-dark font-medium text-sm">
          <ScrollText size={16} />
        ...
        <p className="text-lg md:text-xl text-outline dark:text-gray-400 max-w-lg mx-auto leading-relaxed">
           精准考点 • 智能批改 • 深度问答
        </p>
        ...
        <div className="pt-16 text-center opacity-80">
           <p className="text-xs text-outline font-medium uppercase tracking-widest mb-2">Designed By</p>
           <a 
             href={AUTHOR_LINK} 
             target="_blank" 
             rel="noopener noreferrer"
             className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/50 dark:bg-black/20 hover:bg-white dark:hover:bg-black/40 transition-colors duration-300 text-sm font-bold text-gray-700 dark:text-gray-300 border border-outline/5"
           >
